...
export class CommitmentService {
  ...
  async noCommitment() {
    const commitment_old = await this.commitmentRepository.find({
      order: { commitmentOrder: 'DESC' },
      take: 1,
    });
    if (!commitment_old || commitment_old.length === 0) {
      return 0;
    }
    const commitment_new = commitment_old[0].commitmentOrder + 1;
  ...
  async remove(id: number): Promise<void> {
    ...
    for (const item of allCommitment) {
      if (item.commitmentOrder > commitment.commitmentOrder) {
        item.commitmentOrder -= 1;
