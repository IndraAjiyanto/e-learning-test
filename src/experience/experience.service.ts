...
export class ExperienceService {
  ...
  async noExperience() {
    const experience_old = await this.experienceRepository.find({
      order: { experience_order: 'DESC' },
      take: 1,
    });
    if (!experience_old || experience_old.length === 0) {
      return 0;
    }
    const experience_new = experience_old[0].experience_order + 1;
  ...
  async remove(id: number): Promise<void> {
    ...
    for (const item of allExperience) {
      if (item.experience_order > experience.experience_order) {
        item.experience_order -= 1;
